      ...
      FROM (
            SELECT  log_dt
                ...
                --   json_extract(result_shop_info, '$.shopAd.powerCall') AS powerCall ,
                   ...
                   , json_extract(result_shop_info, '$.temporaryCloseDate') AS temporaryCloseDate
                ...
                --   json_extract(result_shop_info, '$.temporaryCloseDate.finishAt') AS temporaryCloseDate_finishAt ,
                   , json_extract(result_shop_info, '$.closeDate') AS closeDate              
              ...
              FROM (
                  SELECT log_dt
                   ...
                   WHERE 1=1
                     ...
                     AND log.log_dt >= SUBSTR(TRIM('${*1_조회시작일자_시분초.밀리세컨드*}'), 1, 10)
                     AND log.log_dt <= SUBSTR(TRIM('${*1_조회종료일자_시분초.밀리세컨드*}'), 1, 10)
                     AND log.log_date >= timestamp '${*1_조회시작일자_시분초.밀리세컨드*}' - interval '9' hour
                     AND log.log_date <= timestamp '${*1_조회종료일자_시분초.밀리세컨드*}' - interval '9' hour
                     AND log.log_hour >= CAST(SUBSTR(TRIM('${*1_조회시작일자_시분초.밀리세컨드*}'), 12, 2) AS INTEGER)
                     AND log.log_hour <= CAST(SUBSTR(TRIM('${*1_조회종료일자_시분초.밀리세컨드*}'), 12, 2) AS INTEGER)
...
, region_info AS (   
    ...
    SELECT   
          ...
          , s.shop_no   AS "업소번호"
          , s.shop_nm   AS "업소명"
          , s.category_cd AS "업소_카테고리ID"
          , s.category_nm AS "업소_카테고리명"
          , ua.category_id  AS "요청카테고리ID"
          , ua.kst_log_date AS "요청시간"              
            --  , ua.log_dt
        --   , ua.log_date
        --   , 'ua-->'
        --   , ua.region_small_cd
        ...
        --   , r.region_middle_cd
        --   , r.region_middle_nm              
          , r.region_small_cd   AS "요청소지역코드"
          , r.region_small_nm   AS "요청소지역명"
          , ua.page AS "요청페이지"
        --   , ua.request  as "요청내용"
        --   , ua.result_shop_id
        --   , ua.result_shop_info
          ...
          , ua.shopAdsInService
          , ua.shopScore
          , ua.temporaryCloseDate
          , ua.closeDate
          , ua.useBaropay
          , ua.blocked
          , ua.shopOpened
          , IF('${6_노출업전체출력=N,Y|N}' = 'N', null, ua.resultshops) AS "노출업소전체"
      FROM  user_activity ua
          , shop_info s
          , region_info r
     WHERE 1=1
      AND ua.result_shop_id = s.shop_no
      AND ua.region_small_cd = r.region_small_cd
      AND r.region_small_cd = ua.region_small_cd
 ORDER BY ua.ranking, ua.kst_log_date
